<template>
  <div>
    ...
    <b-card  class="my-2" v-for="demande in demandes" :key="demande.id">
      <b-icon class="float-end" v-b-tooltip.hover="{
        variant: 'success',
        title: 'Demande accepted',
        placement: 'topright',
      }" v-if="demande.status == 1" icon="check-square" scale="2" variant="success"></b-icon>
      <b-icon class="float-end" v-b-tooltip.hover="{
        variant: 'danger',
        title: 'Demande not accepted',
        placement: 'topright',
      }" v-if="demande.status == 2" icon="x-square" scale="2" variant="danger"></b-icon>
      <b-icon class="float-end" v-b-tooltip.hover="{
        variant: 'warning',
        title: 'Demande pending',
        placement: 'topright',
      }" v-if="demande.status == 0" icon="x-square" scale="2" variant="warning"></b-icon>
      <md-tabs style="height: auto;!important">
        <md-tab id="tab-home" style="height: auto;!important" md-label="demande">
          <div class="bv-example-row text-center">
            ...
            <b-button variant="danger" :class="[demande.status != 0 ? 'disabled' : '']" v-on:click="Delete(demande.id)">
              Delete</b-button>
            <b-button variant="warning" :class="[demande.status != 0 ? 'disabled' : '']" v-on:click="Update(demande)">
              Update</b-button>
          </div>
        </md-tab>
      </md-tabs>
