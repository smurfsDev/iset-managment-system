<template>
  <div>
    <formReclamation @addReclamation="addReclamation" :oldReclamation="Reclamation" />
            ...
            <b-row class="text-center">
              <b-col cols="8">
                ...
                type="button" class="btn btn-primary mx-1 float-start" data-bs-toggle="modal"
                  @click="initModal()" data-bs-target="#ReclamationModal">
...
export default {
  ...
  methods: {
    fetchReclamation(
      page_url = "http://127.0.0.1:8000/api/reclamation/"
    ) {
      let vm = this;
    ...
    deleteReclamation(id) {
      if (confirm("Delete Reclamation " + id)) {
        this.show = true;
        this.$http.delete("http://localhost:8000/api/reclamation/" + id)
        .then(() => {
          this.fetchReclamation();
    ...
    addReclamation(Reclamation) {
      ...
      this.show = true;
      if (!this.edit) {
       this.$http.post('http://localhost:8000/api/reclamation',
        (Reclamation))
        .then((data) => {
          data = data.data;
        ...
        this.$http.put("http://localhost:8000/api/reclamation/" + Reclamation.id,
        (Reclamation))
        .then(() => {
          this.fetchReclamation();
            this.edit = false;
